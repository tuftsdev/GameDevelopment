...
# Set up our caveman sprite
cman = Caveman(screen, 10, 10)

# Initialize our score
score = 0

# Set up a list of steaks so more than 1 on the screen can be displayed; add a new steak to list every 2 seconds
steaks = []
pygame.time.set_timer(USEREVENT + 1, 2000) # Custom event
...
while True:
	...
	# For each steak in list...
	for s in steaks:
	
		# Check for collision between caveman and steak
		if pygame.sprite.collide_rect(cman, s) and s.eaten == False:
			s.is_eaten() # If the caveman hit the steak, change the state of the steak to eaten and play sound
			score = score + 1
	...
	for event in pygame.event.get():
		if event.type == KEYDOWN:
			if event.key == K_ESCAPE:
				...
				pressed = None
		elif event.type == USEREVENT + 1: # Add a new steak to list every 2 seconds
			steaks.append(Steak(screen, random.randint(50, 750), random.randint(50, 550)))
